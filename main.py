...
openai_api_key = st.secrets[
    "OPENAI_API_KEY"
]  # streamlit/.streamlit/secrets.toml 파일에 정의된 OPENAI_API_KEY 활용
...
if (
...
):  # session_state에 messages가 없으면 빈 리스트 생성
    ...
    for (
        ...
        st.session_state.messages
    ):  # session_state에 messages가 있으면 모든 메시지를 순회
        with st.chat_message(
            message["role"]
        ):  # 메시지 컨테이너(내용물을 담을 수 있는 공간)를 생성, message['role'] 값은 'human' 또는 'ai'로 둘 중 무엇이냐에 따라 컨테이너 디자인이 달라짐
...
if user_input:
    ...
    )  # 사용자 메시지를 session_state에 추가(사용자가 입력한 메시지의 role은 "human"으로 설정)
    ...
    )  # 챗봇 응답을 session_state에 추가(챗봇 메시지의 role은 "ai"로 설정)
